package com.project.codinviec_core_service.specification;
...
public class JobSpecification {
    ...
    public Specification<Job> searchByKeyword(String keyword) {
        return (root, query, cb) -> {
            if (keyword == null || keyword.trim().isEmpty()) return cb.conjunction();
            ...
            try {
                ...
                String pattern = "%" + kw + "%";
                Predicate byJobPosition = cb.like(cb.lower(root.get("jobPosition")), pattern);
                Join<Object, Object> companyJoin = root.join("company", JoinType.LEFT);
                Predicate byCompanyName = cb.like(cb.lower(companyJoin.get("name")), pattern);
                return cb.or(byJobPosition, byCompanyName);
            }
        };
    }
